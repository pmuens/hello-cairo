...
func verify_adjacent_locations(loc0 : Location*, loc1 : Location*):
    ...
    if row_diff == 0:
        ...
        # Verify that the `col` coorinate is the same.
        assert col_diff = 0
        return ()
    end
end
...
func output_initial_values{output_ptr : felt*}(squashed_dict : DictAccess*, n):
    ...
    return output_initial_values(squashed_dict + DictAccess.SIZE, n=n - 1)
end
...
func check_solution{output_ptr : felt*, range_check_ptr}(
    ...
    # Start by veryfing that `loc_list` is valid.
    verify_location_list(loc_list=loc_list, n_steps=n_steps)

    # Allocate memory for the dict and the squased dict.
    ...
    # Verify that the squased dict has exactly 15 entries.
